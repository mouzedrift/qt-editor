    ...
    void AddHoveredObjectsMenu(QMenu* pMenu, QContextMenuEvent* pEvent)
    {
        ...
        for (int i = 0; i < hoveredItems.count(); i++)
        {
            ...
            if (pObjectAction && (pRectItem || pArrowItem))
            {
                ...
                connect(pObjectAction, &QAction::hovered, this, [this, pRectItem, pArrowItem]()
                {
                    auto pOldHoveredMenuItem = mHoveredMenuItem;
                    if (pOldHoveredMenuItem)
                    {
                        pOldHoveredMenuItem->SetHighlighted(false);
                    }

                    if (pRectItem)
                    {
                        mHoveredMenuItem = pRectItem;
                    }
                    else if (pArrowItem)
                    {
                        mHoveredMenuItem = pArrowItem;
                    }

                    mHoveredMenuItem->SetHighlighted(true);
                });
            }
        }

        connect(pHoveredObjectsMenu, &QMenu::aboutToHide, this, [&]()
        {
            if (mHoveredMenuItem)
            {
                mHoveredMenuItem->SetHighlighted(false);
            }
        });
...
private:
    ...
    IGraphicsItem* mHoveredMenuItem = nullptr;
};
